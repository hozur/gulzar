...
class menzil_meta_box{
	...
	function add_script_and_styles(){
		...
		{	
			//注意加载的脚本的url
			wp_enqueue_style('metabox_fields_css', get_bloginfo('template_url') . '/metabox_fields.css');
			wp_enqueue_script('metabox_fields_js', get_bloginfo('template_url') . '/metabox_fields.js');
	...
	function textarea($values){
		...
		$values['height']=($values['height']!='')? $values['height']:10;
		$values['dir']=($values['dir']!='')? $values['dir']:'ltr';
		echo '<p>'.$values['name'].'</p>';
		echo '<p><textarea style="direction:'.$values['dir'].'" class="kriesi_textarea" cols="'.$values['width'].'" rows="'.$values['height'].'" id="'.$values['id'].'" name="'.$values['id'].'">'.$values['std'].'</textarea>';
		echo $values['desc'].'<br/></p>';
	    echo '<br/>';
	}
	//媒体上传
